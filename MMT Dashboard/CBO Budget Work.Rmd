...
```{r Receipts Barchart}
# Prep data
df_totals <- df_cbo_budget_receipts %>%
  group_by(fiscal_year) %>%
  summarize(total_amount = sum(amount), .groups = "drop")

df_plot <- df_cbo_budget_receipts %>%
  left_join(df_totals, by = "fiscal_year") %>%
  mutate(
    pct = amount / total_amount,
    hover_text = paste0(
      "Year: ", fiscal_year,
      "<br>Category: ", category,
      "<br>Amount: $", comma(amount, accuracy = 0.1),
      "<br>Share of Total: ", percent(pct, accuracy = 0.1)
    )
  )

# Create plot
plot_ly(df_plot, x = ~fiscal_year, y = ~amount, color = ~category, type = 'bar',
        text = ~hover_text,
        textposition = "none",  
        hoverinfo = 'text') %>%
  layout(
    barmode = 'stack',
    title = 'Federal Revenue by Category (Interactive)',
